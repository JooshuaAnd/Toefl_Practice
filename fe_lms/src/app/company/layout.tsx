...
import {
  LayoutDashboard, Users, BarChart3, ChevronLeft, Menu, X, Sparkles, LogOut, Building2,
} from "lucide-react";
import { useAppStore } from "@/stores/useAppStore";

const SIDEBAR_ITEMS = [
  { label: "Dashboard", icon: LayoutDashboard, href: "/company/dashboard" },
  { label: "Participants", icon: Users, href: "/company/dashboard/participants" },
  { label: "Reports", icon: BarChart3, href: "/company/dashboard/reports" },
];
...
export default function CompanyLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("linguedu_currentUser");
    if (!stored) {
  ...
  return (
    <div className="relative min-h-screen w-full bg-white text-slate-900 selection:bg-brand-light/30 selection:text-brand-dark">
      <div className="bg-glow-container">
        <div className="bg-glow-1" /><div className="bg-glow-2" /><div className="bg-glow-3" /><div className="bg-grid-overlay" />
      </div>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full bg-white/60 backdrop-blur-2xl border-r border-white/30 shadow-[4px_0_24px_-8px_rgba(0,132,255,0.06)] transition-all duration-300 flex flex-col ${
            ...
            return (
              <Link key={item.href} href={item.href}
                ...
                <div className={`h-9 w-9 rounded-lg flex items-center justify-center transition-all ${
                  isActive ? "bg-brand-main text-white shadow-sm shadow-brand-main/20" : "bg-slate-50 text-slate-400 group-hover:bg-slate-100"
                }`}>
